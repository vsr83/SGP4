...
\end {eqnarray}
\subsubsection{Canonical Momentums}
...
\begin {eqnarray}
  ...
  \begin {array}{lll}
    l = M,           & L = \sqrt{a\mu},                & \\
    g = \omega,\quad & G = \sqrt{a\mu(1 - e^2)}\cos I, & F = -\dfrac{\mu}{2L^2}\\
    h = \Omega,      & H =  \sqrt{a\mu(1 - e^2)},      &
  \end {array}
  }
\end {eqnarray}
...
canonical coordinates. The canonical momentums in the resulting Hamiltonian are then
...
\begin {figure}[h]
  \begin {center}
    \begin{tikzpicture}
      ...
      \draw (4.3, 0.75) node[below] {short-period terms (l)};
      \draw (4.3, 2.25) node[below] {long-period terms (g)};
    \end {tikzpicture}
